...
def investing_strategy2(args):
    ...
    symbol, buy_rate, sell_rate, klines, div, nth_span = args
    # read data
    # content = get_data()
    ...
    # symbol, buy_rate, sell_rate, klines, div, nth_span

    print(f"{param=}")
    regression_result.append(param)


# investing_strategy2(("603119", 0.03, 0.08))
# investing_strategy2(("603119", 0.03, 0.09))

def save_regression_result(symbol, result, buy_rate, sell_rate, parent_path="."):
    import pandas as pd
    import os
    os.makedirs(parent_path, exist_ok=True)
    save_path = f"{parent_path}/regression_result_{buy_rate}_{sell_rate}_{symbol}.xlsx"
    head = list(result[0].keys())
    data = [list(e.values()) for e in result]
    with pd.ExcelWriter(save_path) as ew:
        df = pd.DataFrame(data)
        df.to_excel(ew, header=tuple(head))
    print(f"The result has been saved to {save_path}")


symbol = "603119"
content = get_data(symbol)
klines = content.get("data").get("klines")
klines = convert_kline_entry_to_li(klines)
length = len(klines)
regression_result = []
buy_rate = 0.04
sell_rate = 0.09
for div in [5, 10, 20, 30, 40, 50]:
    # division = 30
    span = length // div
    for i in range(div):
        investing_strategy2(("603119", buy_rate, sell_rate, klines[i*span:], div, i))
    # investing_strategy2(("603119", 0.03, 0.08, klines[10*span:]))

save_regression_result(symbol, regression_result, buy_rate, sell_rate)