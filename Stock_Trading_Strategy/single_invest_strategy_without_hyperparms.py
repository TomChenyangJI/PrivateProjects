...
def investing_strategy():
    ...
    # import pandas as pd
    # with pd.ExcelWriter("./mavg.xlsx", mode="w") as ew:
    #     df = pd.DataFrame(mavg_5)
    #     df.to_excel(ew, "mavg5")
    #     df = pd.DataFrame(mavg_10)
    #     df.to_excel(ew, "mavg10")
    ...
    while i < length:
        ...
        if mavg_5_val >= mavg_10_val:
            if prev_state is False:
                buy = True  # this is the moment to buy the stock
                sell = None
            prev_state = True

        else:  # mavg_5_val < mavg_10_val
            if prev_state:
                sell = True  # this is the moment to sell the stock
                buy = None
            prev_state = False
        i += 1

    print(round(capital + amount * open_price, 2), f"{trade_times=}")


# investing_strategy()
# TODO include the hyper variable to training the model tmr
...
# investing_strategy2(("603119", 0.1, 0.2))


def param_generator(symbol="603119", buy_up=50, sell_up=50):
    for buy_point in range(buy_up):
        buy_point /= 100
        for sell_point in range(sell_up):
            sell_point /= 100
            yield symbol, buy_point, sell_point


def initializer(que, lock, val):
    global q, l, v
    q = que
    l = lock
    v = val
...
if __name__ == "__main__":
    ...
    v = Value("i", 0)
    symbol = "603119"
    with Pool(30, initializer=initializer, initargs=(q, l, v)) as p:
        # p.map(investing_strategy2, param_generator())
        p.map(investing_strategy2, [("603119", 0.1, 0.1), ])
        try:
            save_result(symbol, q)
        except Exception:
            pass
