...
def investing_strategy_with_hyperparams(args):
    ...
    try:
        ...
        # print(f"{symbol} has not been conducted experiments on becuase {e} happened.")
        with l:
            error_msg = f"{symbol} has not been conducted experiments on becuase {e} happened."
            # exception_storage_temp = exception_que.get()
            # print(type(exception_storage_temp))
            # print(1, exception_storage_temp)
            # if error_msg not in exception_storage_temp:
            #     exception_storage_temp.append(exception_storage_temp)
            # print(2, error_msg, exception_storage_temp)
...
def param_generator(symbol, symbol_data_json_file, buy_up=50, sell_up=50):
    ...
    for buy_point in range(buy_up):
        ...
        for sell_point in range(sell_up):
            sell_point /= 100
            yield symbol, symbol_data_json_file, buy_point, sell_point
...
if __name__ == "__main__":
    ...
    with open(symbol_name_mapping_path) as fi:
        ...
        for i, symbol in enumerate(symbols):
            ...
            v = Value("i", 0)
            # symbol = "600285"
            with Pool(35, initializer=initializer, initargs=(q, l, v, exception_que)) as p:
                p.map(investing_strategy_with_hyperparams, param_generator(symbol, symbol_data_json_file))
                # p.map(investing_strategy_with_hyperparams, [("603119", 0.1, 0.1), ])
                try:
                    save_result(symbol, q)
                except Exception:
                    pass
